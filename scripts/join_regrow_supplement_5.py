...
output_folder_Regrow = Path("data/edited/Regrow")
...
for state in states:
    ...
    output_path_geojson = (
        output_folder_Regrow / f"{state}_regrow_supplement_5_spatial.geojson"
    )
    output_path_csv = output_folder_Regrow / f"{state}_regrow_supplement_5_table.csv"
    ...
    columns_to_keep = [
        c
        for c in regrow_shape.columns
        if (c == "field_id")
        or (c == "area_acre")
        or (c == "geometry")
        or c.startswith("PHtill_1")
        or c.startswith("PHtill_2")
        or c.startswith("PPtill_1")
        or c.startswith("PPtill_2")
        or c.startswith("cover_1")
        or c.startswith("cover_2")
        or c.startswith("crop_1")
        or c.startswith("crop_2")
    ]
    regrow_shape = regrow_shape[columns_to_keep]

    # Identify neighboring fields
    neighbor_gdf = gpd.sjoin(
        regrow_shape,
        regrow_shape,
        how="left",
    ...
    neighbor_gdf = neighbor_gdf[
        neighbor_gdf["field_id_self"] != neighbor_gdf["field_id_nbr"]
    ]
    ...
    for new_col, cols in nbr_activity_dict.items():
        ...
        neighbor_till_cover[new_col] = neighbor_till_cover["field_id_self"].map(
            mean_values
        )
    ...
    crop_cols = [
        c for c in neighbor_gdf.columns if c.startswith("crop_") and c.endswith("_nbr")
    ]

    crop_groups = {"corn": [1], "soybean": [5], "wheat": [22, 23, 24]}
    ...
    for col in crop_cols:
        ...
        denom = (
            neighbor_gdf.loc[mask_valid].groupby("field_id_self")["area_acre_nbr"].sum()
        )
        ...
        for group_name, codes in crop_groups.items():
            ...
            numer = (
                neighbor_gdf.loc[mask_group]
                .groupby("field_id_self")["area_acre_nbr"]
                .sum()
            )
            numer = numer.reindex(denom.index, fill_value=0)  # align with denom
            col_df[f"crop_{group_name}_nbr_{col.split('_')[1]}_{col.split('_')[2]}"] = (
                np.where(denom != 0, numer / denom, np.nan)
            )

        # Compute "other"
        mask_other = mask_valid & ~neighbor_gdf[col].isin(all_group_codes)
        numer = (
            neighbor_gdf.loc[mask_other].groupby("field_id_self")["area_acre_nbr"].sum()
        )
        numer = numer.reindex(denom.index, fill_value=0)
        col_df[f"crop_other_nbr_{col.split('_')[1]}_{col.split('_')[2]}"] = np.where(
            denom != 0, numer / denom, np.nan
        )
    ...
    regrow_supplement_5 = regrow_supplement_5.merge(
        neighbor_till_cover, left_on="field_id", right_on="field_id_self", how="left"
    )
    regrow_supplement_5 = regrow_supplement_5.merge(
        neighbor_crop, left_on="field_id", right_on="field_id_self", how="left"
    )
    regrow_supplement_5 = regrow_supplement_5.drop(
        columns=["field_id_self_x", "field_id_self_y"]
    )
