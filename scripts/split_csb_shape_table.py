...
def split_csb_to_shape_table(state, year, target_CRS, CSB_input_folder, CSB_output_folder):
    # Input and outpur file names
    input_path = os.path.join(CSB_input_folder, f"{state}_CSB{year}_clipped.parquet")
    output_path_joined = os.path.join(CSB_output_folder, f"{state}_CSB{year}_shape_table.parquet")
    output_path_fieldID_geometry_parquet = os.path.join(CSB_output_folder, f"{state}_CSB{year}_CSBID_geometry.parquet")
    output_path_fieldID_geometry_gpkg = os.path.join(CSB_output_folder, f"{state}_CSB{year}_CSBID_geometry.gpkg")
    output_path_table = os.path.join(CSB_output_folder, f"{state}_CSB{year}_table.parquet")
...
for year in years:
    for state in states:
        split_csb_to_shape_table(state, year, target_CRS, CSB_input_folder, CSB_output_folder)