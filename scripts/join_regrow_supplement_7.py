...
for state_regrow in states:
    # Path to Regrow file
    input_path_Regrow = os.path.join(
        input_folder_Regrow, f"{state_regrow}_regrow_supplement_1_spatial.geojson"
    )
    # Load regrow joined datasets
    regrow_supplement_1 = gpd.read_file(input_path_Regrow)
    # Replace St to Saint in the county_name column
    regrow_supplement_1["county_name"] = regrow_supplement_1["county_name"].str.replace(
    ...
    for crop in crops:
        ...
        for state in states:
            df_elevator_price = pd.read_excel(
                f"data/edited/Grain Price/{state}_{crop}_elevator_level.xlsx",
                sheet_name="Values",
                header=[0, 1],
            )
            df_elevator_price[("History", "Date")] = pd.to_datetime(
                df_elevator_price[("History", "Date")]
            )

            new_cols = {}
            for col in df_elevator_price.columns.get_level_values(0)[1:]:
                new_cols[(col, "Average")] = (
                    df_elevator_price[(col, "Low")] + df_elevator_price[(col, "High")]
                ) / 2
            avg_df_elevator_price = pd.DataFrame(
                new_cols, index=df_elevator_price.index
            )
            df_elevator_price = pd.concat(
                [df_elevator_price, avg_df_elevator_price], axis=1
            )

            df_elevator_price.columns = [
                f"{ticker}_{price}" for ticker, price in df_elevator_price.columns
            ]

            df_elevator_price.rename(columns={"History_Date": "Date"}, inplace=True)

            df_crop_elevator = df_crop_elevator.merge(
                df_elevator_price, on="Date", how="left"
            )

        # Drop duplicate column names (keep the first occurence)
        df_crop_elevator = df_crop_elevator.loc[
            :, ~df_crop_elevator.columns.duplicated()
        ]
        # Select only Date and columns with average elevator prices
        df_crop_avg = df_crop_elevator.loc[
            :, df_crop_elevator.columns.str.endswith(("Date", "Average"))
        ]
        # Remove "Average" from column names
        df_crop_avg.columns = df_crop_avg.columns.str.replace(
            "_Average", "", regex=False
        )
        ...
        elevator_location = gpd.read_file(
            f"data/edited/Grain Price/{crop}_elevator_location.geojson"
        )
        elevator_location = elevator_location[~elevator_location.geometry.isna()]

        # Compute price of the nearest elevator
        gdf_parcel = regrow_supplement_1.to_crs(epsg=5070)
        ...
        elevator_coords = np.array(
            [[geom.x, geom.y] for geom in elevator_location.geometry]
        )
        ...
        value_dict = {
            row["ticker"]: row[month_cols] for _, row in df_crop_avg.iterrows()
        }
        ...
        # Rename columns
        gdf_parcel_price.columns = [
            col.replace(f"{crop}_price_elevator_", f"{crop}_price_elevator_nearest_")
            if col.startswith(f"{crop}_price_elevator_")
            else col
            for col in gdf_parcel_price.columns
        ]

        # Save price file
        temp_file_path = os.path.join(
            temp_dir, f"{state_regrow}_{crop}_price_nearest.csv"
        )
        ...
        weights_masked = weights_expanded * mask

        # Normalize weights along N axis so sum = 1 for available values
        weight_sums = weights_masked.sum(axis=1, keepdims=True)
        normalized_weights = np.divide(
            weights_masked, weight_sums, where=weight_sums != 0
        )

        weighted_mean = np.nansum(data_stack * normalized_weights, axis=1)

        weighted_prices = pd.DataFrame(
            weighted_mean, columns=all_nearest_elevators[0].columns
        )
        weighted_prices.replace(0, np.nan, inplace=True)
        weighted_prices.insert(0, "field_id", gdf_parcel["field_id"])
        weighted_prices.columns = [
            col.replace(
                f"{crop}_price_elevator_", f"{crop}_price_elevator_{N}-nearest_"
            )
            if col.startswith(f"{crop}_price_elevator_")
            else col
            for col in weighted_prices.columns
        ]

        # Save price file
        temp_file_path = os.path.join(
            temp_dir, f"{state_regrow}_{crop}_price_{N}-nearest.csv"
        )
        weighted_prices.to_csv(temp_file_path, index=False)

        # Remove heavy variables from memory
        del data_stack, weights_expanded, normalized_weights

        print(f"Elevator-level {crop} price in {state_regrow} is computed and saved.")

        # ---# County-level crop price
        if crop != "wheat":
            for state in states:
                df_county_price = pd.read_excel(
                    f"data/edited/Grain Price/{state}_{crop}_county_level.xlsx",
                    sheet_name="Values",
                    header=[0, 1],
                )
                df_county_price[("History", "Date")] = pd.to_datetime(
                    df_county_price[("History", "Date")]
                )

                new_cols = {}
                for col in df_county_price.columns.get_level_values(0)[1:]:
                    new_cols[(col, "Average")] = (
                        df_county_price[(col, "Low")] + df_county_price[(col, "High")]
                    ) / 2
                avg_df_county_price = pd.DataFrame(
                    new_cols, index=df_county_price.index
                )
                df_county_price = pd.concat(
                    [df_county_price, avg_df_county_price], axis=1
                )

                df_county_price.columns = [
                    f"{ticker}_{price}" for ticker, price in df_county_price.columns
                ]

                df_county_price.rename(columns={"History_Date": "Date"}, inplace=True)

                df_crop_county = df_crop_county.merge(
                    df_county_price, on="Date", how="left"
                )
            ...
            df_crop_county_avg = df_crop_county.loc[
                :, df_crop_county.columns.str.endswith(("Date", "Average"))
            ]
            # Remove "Average" from column names
            df_crop_county_avg.columns = df_crop_county_avg.columns.str.replace(
                "_Average", "", regex=False
            )
            # Select values from January 2014 onwards
            df_crop_county_avg = df_crop_county_avg[
                df_crop_county_avg["Date"] >= "2014-01-01"
            ]
            ...
            cols_to_keep = (
                df_crop_county_avg.loc[mask].dropna(axis=1, thresh=12).columns
            )
            df_crop_county_avg = df_crop_county_avg.loc[:, cols_to_keep]

            # Set first column as index
            df_crop_county_avg = df_crop_county_avg.set_index(
                df_crop_county_avg.columns[0]
            )
            # Transpose
            ...
            # Reset index so original column names become first column
            df_crop_county_avg = df_crop_county_avg.reset_index().rename(
                columns={"index": "ticker"}
            )

            index_county_location = gpd.read_file(
                f"data/edited/Grain Price/{crop}_index_county_location.geojson"
            )
            index_county_location = index_county_location[
                ~index_county_location.geometry.isna()
            ]
            ...
            centroids = gdf_parcel.geometry.centroid
            parcel_coords = np.column_stack((centroids.x, centroids.y))
            county_coords = np.array(
                [[geom.x, geom.y] for geom in index_county_location.geometry]
            )
            ...
            county_name_to_index = dict(
                zip(index_county_location["county"], index_county_location.index)
            )
            ...
            # Dictionary: ticker -> DataFrame of months (1 row per ticker)
            value_dict = {
                row["ticker"]: row[month_cols]
                for _, row in df_crop_county_avg.iterrows()
            }
            ...
            # Save price file
            temp_file_path = os.path.join(
                temp_dir, f"{state_regrow}_{crop}_price_county_index.csv"
            )
            ...
            print(f"County-level {crop} price in {state_regrow} is computed and saved.")
...
for state_regrow in states:
    ...
    csv_files = sorted(temp_dir.glob(f"{state_regrow}_*.csv"))
    ...
    all_crop_prices.to_csv(
        f"data/edited/Regrow/{state_regrow}_regrow_supplement_7_table.csv", index=False
    )
    print(f"Saved for {state_regrow}")
