...
for state_regrow in states:
    ...
    # Path to Regrow file
    input_path_Regrow = os.path.join(input_folder_Regrow, f"{state_regrow}_regrow_supplement_1_spatial.geojson")
    ...
    for crop in crops:
        ...
        # Save price file
        temp_file_path = os.path.join(temp_dir, f"{state_regrow}_{crop}_price_nearest.csv")
        ...
        normalized_weights = np.divide(weights_masked, weight_sums, where=weight_sums!=0)
        ...
        # Save price file
        temp_file_path = os.path.join(temp_dir, f"{state_regrow}_{crop}_price_{N}-nearest.csv")
        ...
        if crop != 'wheat':
            ...
            # Save price file
            temp_file_path = os.path.join(temp_dir, f"{state_regrow}_{crop}_price_county_index.csv")
...
for state_regrow in states:
    ...
    all_crop_prices.to_csv(f'data/edited/Regrow/{state_regrow}_regrow_supplement_7_table.csv', index=False)
