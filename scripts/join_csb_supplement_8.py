...
for state in states:
    ...
    input_path_CSB_geometry = os.path.join(input_folder_CSB, f"{state}_CSB1724_CSBID_geometry.parquet")
    input_path_CSB_raster = os.path.join(input_folder_CSB, f"{state}_CSB1724_raster_to_gSSURGO_grid.tif")
    input_path_duplicating_fields = os.path.join(input_folder_CSB, f"{state}_CSB1724_duplicating_fields.parquet")
    ...
    CSB_geometry = gpd.read_parquet(input_path_CSB_geometry)
    ...
    CSB_overlaps = pd.read_parquet(input_path_duplicating_fields)
    ...
    # Open rasterized CSB
    with rasterio.open(input_path_CSB_raster) as src_CSB:
    ...
    parcel_ids = CSB_raster[rows, cols]
    ...
    print(f"CSBID fields in {state} with no pixels assigned")
