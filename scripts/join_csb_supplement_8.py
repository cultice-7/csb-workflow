...
for year in CSB_years:
    for state in states:
        ...
        CSB_raster_input_path = os.path.join(CSB_input_folder, f"{state}_CSB{year}_raster_to_gSSURGO_grid.tif")
        duplicating_fields_input_path = os.path.join(CSB_input_folder, f"{state}_CSB{year}_duplicating_fields.parquet")
        output_path_spatial = os.path.join(CSB_output_folder, f"{state}_CSB{year}_supplement_8_spatial.parquet")
