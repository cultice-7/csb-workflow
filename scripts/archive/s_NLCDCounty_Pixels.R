...
NLCD_CropSave <- function(year.i,
  ...
  #       full raster data
  nlcd.i     <- terra::rast(here("Data", "NLCD", "Raw", rasters[[1,1]]))
  nlcd.crs.i <- terra::crs(nlcd.i)
  ...
  state.sf.i  <- st_read(state.loc) %>%
    st_transform(., nlcd.crs.i) # read it in
  ...
  f_statecrop <- function(state.i2,
    ...
    nlcd_crop.i2  <- terra::crop(nlcd.i2, state.sv.i2)
    nlcd_crop.i2[is.nan(nlcd_crop.i2)] <- 0
    
    ### Note: Save the cropped file
    tempname <- here("Data", "NLCD", "Raw", paste0("NLCD_", state.i2, "_", year.i2, ".tif"))
    terra::writeRaster(nlcd_crop.i2, tempname, 
                       overwrite = TRUE)
    return(paste0("Crop for ", state.i2, year.i2, " complete."))
  }
  map(state.v.i, f_statecrop)
}
map(inp1, NLCD_CropSave)
