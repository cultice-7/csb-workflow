...
for state in states:
    ...
    output_path_geojson = (
        output_folder_Regrow / f"{state}_regrow_supplement_6_spatial.geojson"
    )
    output_path_csv = output_folder_Regrow / f"{state}_regrow_supplement_6_table.csv"
    ...
    for variable in weather_variables:
        ...
        regrow_shape = regrow_shape.join(
            pd.DataFrame(new_cols, index=regrow_shape.index)
        )
