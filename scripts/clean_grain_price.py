...
for crop in crops:
    ...
    for state in states:
        df_elevator_price = pd.read_excel(
            f"data/Grain Price/{state}_{crop}_elevator_level.xlsx",
            sheet_name="Values",
            header=[0, 1],
        )
    ...
    df_crop_avg.columns = [
        f"{crop}_price_elevator_{col.strftime('%Y%m')}" for col in df_crop_avg.columns
    ]
    ...
    for col in df_crop_elevator.columns:
        if col.endswith("_Name"):
            ...
            value = (
                df_crop_elevator[col].dropna().unique()[0]
                if df_crop_elevator[col].notna().any()
                else None
            )
    ...
    elevator_location = pd.DataFrame({"ticker": tickers, "elevator description": names})
    elevator_location = elevator_location[
        elevator_location["ticker"].isin(df_crop_avg["ticker"])
    ]
    ...
    elevator_location["elevator location"] = (
        elevator_location["elevator description"].str.split(";").str[1].str.strip()
    )
    # Replace "St" and "St." with "Saint" in elevator location
    elevator_location["elevator location"] = elevator_location[
        "elevator location"
    ].str.replace(r"\bSt\.?\b", "Saint", regex=True)
    # Replace incorrect elevator locations
    elevator_location_corrected = pd.read_excel(
        f"data/edited/Grain Price/corrected_elevator_location_corn_soybeans_wheat.xlsx"
    )
    elevator_location["elevator location"] = (
        elevator_location["ticker"]
        .map(elevator_location_corrected.set_index("ticker")["corrected location"])
        .fillna(elevator_location["elevator location"])
    )
    # Apply geocode_location to find the centroids of towns where elevators are located
    elevator_location[["geometry", "county", "state"]] = elevator_location[
        "elevator location"
    ].apply(lambda x: pd.Series(geocode_location(x)))
    # Extract crop type from elevator description
    elevator_location["crop type"] = (
        elevator_location["elevator description"]
        .str.split(";")
        .str[2]
        .str.split()
        .str[0]
    ...
    elevator_location = gpd.GeoDataFrame(
        elevator_location, geometry="geometry", crs="EPSG:4326"
    )

    # Verify that the crop type listed for elevators match the crop type we are currently working with
    crop_type_check = (elevator_location["crop type"].notna()) & (
        elevator_location["crop type"] != crop
    )
    ...
    elevator_location.to_file(
        f"data/edited/Grain Price/{crop}_elevator_location.geojson", driver="GeoJSON"
    )
    elevator_location.to_excel(
        f"data/edited/Grain Price/{crop}_elevator_location.xlsx", index=False
    )
    ...
    if crop != "wheat":
        for state in states:
            df_county_price = pd.read_excel(
                f"data/Grain Price/{state}_{crop}_county_level.xlsx",
                sheet_name="Values",
                header=[0, 1],
            )
        ...
        for col in df_crop_county.columns:
            if col.endswith("_Name"):
                ticker = col.split("_")[0]
                value = (
                    df_crop_county[col].dropna().unique()[0]
                    if df_crop_county[col].notna().any()
                    else None
                )
                tickers.append(ticker)
        ...
        index_county_location = pd.DataFrame(
            {"ticker": tickers, "index description": indices}
        )
        index_county_location = index_county_location[
            index_county_location["ticker"].isin(df_crop_county_avg["ticker"])
        ]
        ...
        index_county_location["county"] = (
            index_county_location["index description"].str.split(",").str[0].str.strip()
        )
        # Replace "St" and "St." with "Saint" in county name
        index_county_location["county"] = index_county_location["county"].str.replace(
            r"\bSt\.?\b", "Saint", regex=True
        )
        # Extract state name from index description
        index_county_location["state"] = (
            index_county_location["index description"]
            .str.split(",")
            .str[1]
            .str.split()
            .str[0]
        ...
        index_county_location[["geometry"]] = (
            index_county_location["county"] + ", " + index_county_location["state"]
        ).apply(lambda x: pd.Series(geocode_location(x)))
        # Extract crop type from price index description
        index_county_location["crop type"] = (
            index_county_location["index description"]
            .str.split(",")
            .str[1]
            .str.split()
        ...
        index_county_location["crop type"] = index_county_location[
            "crop type"
        ].str.replace("soybean", "soybeans", regex=False)
        # Create a GeoDataFrame with counties location
        index_county_location = gpd.GeoDataFrame(
            index_county_location, geometry="geometry", crs="EPSG:4326"
        )

        # Verify that the crop type listed for price index match the crop type we are currently working with
        crop_type_check = (index_county_location["crop type"].notna()) & (
            index_county_location["crop type"] != crop
        )
        if crop_type_check.any():
            print("Warning: crop type mismatch detected")
        ...
        index_county_location.to_file(
            f"data/edited/Grain Price/{crop}_index_county_location.geojson",
            driver="GeoJSON",
        )
        index_county_location.to_excel(
            f"data/edited/Grain Price/{crop}_index_county_location.xlsx", index=False
        )
