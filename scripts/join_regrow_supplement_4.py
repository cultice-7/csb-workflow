...
output_folder_road = Path("data/edited/road")
...
for state in states:
    ...
    output_path_geojson = (
        output_folder_Regrow / f"{state}_regrow_supplement_4_spatial.geojson"
    )
    output_path_csv = output_folder_Regrow / f"{state}_regrow_supplement_4_table.csv"
    output_path_road_geojson = (
        output_folder_road / f"{state}_regrow_points_on_road.geojson"
    )
    output_path_road_csv = (
        output_folder_road / f"{state}_regrow_points_on_road_table.csv"
    )
    ...
    try:
        ...
        regrow_supplement_4 = gpd.sjoin_nearest(
            regrow_shape, roads, how="left", distance_col="dist_to_road"
        )
        ...
        regrow_supplement_4["road_priority"] = regrow_supplement_4["MTFCC"].map(
            priority
        )
        ...
        regrow_supplement_4 = regrow_supplement_4.sort_values(
            ["field_id", "dist_to_road", "road_priority"], ascending=[True, True, True]
    ...
    if regrow_supplement_4[cols_to_check].isna().any().any():
        print(
            regrow_supplement_4[regrow_supplement_4[cols_to_check].isna().any(axis=1)]
        )
    ...
    try:
        ...
        point_on_road_gdf = gpd.GeoDataFrame(
            ...
            geometry=gpd.points_from_xy(
                nearest_points_df["road_x"], nearest_points_df["road_y"]
            ),
            crs=roads.crs,
        )
