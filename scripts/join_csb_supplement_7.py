...
for state_CSB in states:
    # Path to CSB file
    input_path_CSB = os.path.join(
        input_folder_CSB, f"{state_CSB}_CSB1724_supplement_1_spatial.geojson"
    )
    # Load CSB joined datasets
    CSB_supplement_1 = gpd.read_file(input_path_CSB)
    # Replace St to Saint in the county_name column
    CSB_supplement_1["county_name"] = CSB_supplement_1["county_name"].str.replace(
    ...
    for crop in crops:
        ...
        normalized_weights = np.divide(
            weights_expanded, weight_sums, where=weight_sums != 0
        )
        ...
        temp_file_path = os.path.join(
            temp_dir, f"{state_CSB}_{crop}_price_{N}-nearest.csv"
        )
        ...
        if crop != "wheat":
            ...
            # Save price file
            temp_file_path = os.path.join(
                temp_dir, f"{state_CSB}_{crop}_price_county_index.csv"
            )
...
for state_CSB in states:
    ...
    all_crop_prices.to_csv(
        f"data/edited/CSB/{state_CSB}_CSB1724_supplement_7_table.csv", index=False
    )
