...
def add_county_state_name(df, state):
    ...
    # Build the county_state_name key, matching the Regrow-side key (county_name + state)
...
# =============================================================================
# Main script: for each state, build the county-level USDA Ag Census table,
# reshape it to wide format, and attach it to the sorted list of counties
# present among that state's Regrow fields (county-level output, keyed on
# county_state_name -- not merged onto individual fields)
# =============================================================================
for state in states:
    ...
    regrow_input_path = os.path.join(regrow_input_folder, f"{state}_regrow_census_tract_table.parquet")
    regrow_supplement_1 = pd.read_parquet(regrow_input_path)
    ...
    regrow_supplement_9 = counties.merge(census_county_data_wide, on='county_state_name', how='left')

    # Convert float64 columns to float32 to save memory
    float64_cols = regrow_supplement_9.select_dtypes(include=["float64"]).columns
    regrow_supplement_9[float64_cols] = regrow_supplement_9[float64_cols].astype("float32")

    # Save attribute table
    output_path_table = os.path.join(regrow_output_folder, f"{state}_regrow_ag_census_table.parquet")
    regrow_supplement_9.to_parquet(output_path_table, index=False, compression="zstd")
    print(f'Supplementary data 9 for {state} is created and saved')
