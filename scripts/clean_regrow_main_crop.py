...
cols_to_convert = ['plant_date', 'harvest_date']
...
df['planting_days'] = (df['harvest_date'] - df['plant_date']).dt.days

# Sort dataset by boundary_id, plant_year, plant_date (so that earlier plantings within each year come first)
df.sort_values(by = ['boundary_id', 'plant_year', 'plant_date'], inplace = True)
df.reset_index(drop = True, inplace = True)

# Multicropping detection
df['multicrop'] = df.groupby(['boundary_id', 'plant_year']).cumcount() + 1
...
df['year_cropset'] = df['plant_year'].astype(str).str[2:] + '_' + df['multicrop'].astype(str)

# Drop unnecessary columns
#df.drop(columns = ['plant_date', 'harvest_date', 'crop_type_confidence', 'plant_year', 'harvest_year', 'multicrop'], inplace=True)

# Reshape from long to wide
df_wide = df.pivot(index = 'boundary_id', columns = 'year_cropset', values = 'practice_determination')
df_wide.reset_index(inplace = True)
df_wide.columns = ['field_id'] + ['main' + str(col) for col in df_wide.columns[1:]]
