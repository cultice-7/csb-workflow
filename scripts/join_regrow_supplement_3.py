...
for state in states:
    ...
    output_path_geojson = (
        output_folder_Regrow / f"{state}_regrow_supplement_3_spatial.geojson"
    )
    output_path_csv = output_folder_Regrow / f"{state}_regrow_supplement_3_table.csv"
    ...
    for code in hu_codes:
        if code == "8":
            ...
            df_hu = df_hu.drop_duplicates(
                subset=[f"huc{code}", "geometry"]
            ).reset_index(drop=True)
            cols = [f"huc{code}", "name", "geometry"]
            df_hu = df_hu[cols]
            df_hu.rename(
                columns={f"huc{code}": "subbasin_id", "name": "subbasin_name"},
                inplace=True,
            )
        elif code == "10":
            ...
            df_hu.rename(
                columns={
                    f"huc{code}": "watershed_id",
                    "name": "watershed_name",
                    "hutype": "watershed_type",
                },
                inplace=True,
            )
        elif code == "12":
            ...
            df_hu.rename(
                columns={
                    f"huc{code}": "subwatershed_id",
                    "name": "subwatershed_name",
                    "hutype": "subwatershed_type",
                },
                inplace=True,
            )
        ...
        try:
            ...
            cols_to_drop = [
                col for col in regrow_shape.columns if col.endswith("_temp")
            ]
            regrow_shape.drop(columns=cols_to_drop, inplace=True)
            print("Hydrography data added to attribute table.")
