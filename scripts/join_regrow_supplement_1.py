...
tract_boundaries.rename(
    columns={
        "STATEFP": "state_id",
        "STUSPS": "state_name",
        "COUNTYFP": "county_id",
        "NAMELSADCO": "county_name",
        "GEOID": "census_tract_id",
        "ALAND": "tract_land_area",
        "AWATER": "tract_water_area",
    },
    inplace=True,
)
...
for state in states:
    ...
    output_path_geojson = (
        output_folder_Regrow / f"{state}_regrow_supplement_1_spatial.geojson"
    )
    output_path_csv = output_folder_Regrow / f"{state}_regrow_supplement_1_table.csv"
    try:
        ...
        joined = gpd.sjoin(
            parcel_centroids, tract_boundaries, how="left", predicate="within"
        )
        ...
        nearest_tract = gpd.sjoin_nearest(
            joined.loc[unmatched_mask, ["field_id", "geometry"]],
            tract_boundaries,
            how="left",
        ...
        regrow_shape = regrow_shape.merge(
            joined[cols_available], on="field_id", how="left", suffixes=("", "_temp")
        )
