...
def main():
    input_folder = Path("data/edited/Regrow")
    output_folder = Path("data/edited/Regrow/validation")
    output_folder.mkdir(parents=True, exist_ok=True)

    states = snakemake.params.states

    for state in states:
        input_path = input_folder / f"{state}_regrow_shape_table.parquet"
        output_path = output_folder / f"{state}_regrow_with_cdl_validation.geojson"
        output_path_csv_1 = (
            output_folder / f"{state}_regrow_validity_summary_by_year.csv"
        )
        output_path_csv_2 = (
            output_folder / f"{state}_regrow_validity_summary_by_year_cdl_1_5.csv"
        )

        gdf = gp.read_parquet(input_path)
        raster_folder = Path("data/edited/CDL")
        summary_all, summary_cdl_1_5 = [], []

        for year in range(2014, 2025):
            raster_path = raster_folder / f"{year}_30m_cdls_clipped.tif"
            ...
            crop_cols = [
                f"crop{str(year)[2:]}_{i}"
                for i in range(1, 4)
                if f"crop{str(year)[2:]}_{i}" in gdf.columns
            ]
            args_list = [
                (
                    mapping(row.geometry),
                    [row[c] for c in crop_cols if pd.notnull(row[c])],
                    raster_path,
                )
